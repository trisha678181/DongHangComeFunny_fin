...
<div class="container" style="min-height: 479px;">
	...
	<div id="myBoardForm">
		...
		<button id="fcBtn" onclick="location.href='/mypage/commentlist'" style="box-shadow: 4px 4px 4px 1px #00000060 inset;">자유게시판</button>
		<button id="rcBtn" onclick="location.href='/mypage/rcommentlist'">후기게시판</button>
		<button id="deleteBtn" class="deleteCboardBtn" type="button">삭제</button>
		...
		<table id="myBoardTable" class="table table-hover table-bordered">
			...
			<c:forEach items="${cboardData.clist}" var="cboard">				
				<tr>
	...
	<nav aria-label="Page navigation" style="text-align: center;">
		  <ul class="pagination">
		    <li>
		      <a href="<%= request.getContextPath() %>/mypage/commentlist" aria-label="Previous">
		        <span aria-hidden="true">&laquo;</span>
		      </a>
		      
		      <c:choose>
		      	<c:when test="${paging.blockStart > 1 }">
		      		<a href="<%= request.getContextPath() %>/mypage/commentlist?cPage=${paging.currentPage-1}" aria-label="Previous">
		        		<span aria-hidden="true">&lt;</span>
		      		</a>
		      	</c:when>
		      	
		      	<c:when test="${paging.currentPage eq 1 }">
		      		<a href="<%= request.getContextPath() %>/mypage/commentlist?cPage=${paging.blockStart}" aria-label="Previous">
		        		<span aria-hidden="true">&lt;</span>
		      		</a>
		      	</c:when>
		      ...
		      <c:forEach begin="${paging.blockStart}" end="${paging.blockEnd}" var="page">
         			<li><a href="<%= request.getContextPath() %>/mypage/commentlist?cPage=${page}"><span>${page}</span></a></li>
        	  </c:forEach> 
		    ...
		    <li>
		    	<c:choose>
           			<c:when test="${paging.currentPage eq paging.lastPage }">
               			<a href="<%= request.getContextPath() %>/mypage/commentlist?cPage=${paging.blockEnd}" aria-label="Next">
		       	 			<span aria-hidden="true">&gt;</span>
		      			</a>
          			 </c:when>
          		 <c:otherwise>
              			 <a href="<%= request.getContextPath() %>/mypage/commentlist?cPage=${paging.currentPage+1}" aria-label="Next">
		       	 			<span aria-hidden="true">&gt;</span>
		      			</a>
         		 </c:otherwise>
          	 	 </c:choose>
		      ...
		      <a href="<%= request.getContextPath() %>/mypage/commentlist?cPage=${paging.lastPage}" aria-label="Next">
